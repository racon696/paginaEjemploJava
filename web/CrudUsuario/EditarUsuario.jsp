                ...
                <div class="col s8 offset-s2"> 
                    <div class="container white">
                        <table class="highlight-4 responsive-table centered">
                            ...
                            <c:forEach var = "row" items = "${usuarios.rows}">
                                <tr>
                                    <td> <c:out value = "${row.id_usuario}"/></td>
                                    <td> <c:out value = "${row.rut}"/></td>
                                    <td> <c:out value = "${row.nombre}"/></td>
                                    <td> <c:out value = "${row.apellidoP}"/></td>
                                    <td> <c:out value = "${row.apellidoM}"/></td>
                                    <td> <c:out value = "${row.tipousuario}"/></td>
                                    <td> <c:out value = "${row.user}"/></td>
                                    <td> <c:out value = "${row.clave}"/></td>
                                    <td><form action="<c:out value="${pageContext.request.contextPath}"></c:out>/ControladorUsuario" method="POST" ><input type="hidden" name="id_usuario" value="<c:out value="${row.id_usuario}"></c:out>"></input><input type="submit" value="Buscar" name="opcion"></input></form></td>
